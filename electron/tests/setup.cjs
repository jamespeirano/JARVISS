...
def maps(progress,cancel=None):
 (ROOT/'map-ready').touch();return ROOT/'fixture.pmtiles'
setup.prepare_us=maps
...
`);
  ...
  console.log('PASS: setup recommendation, model selection, low storage, two window sizes, pause/reload/retry, all components, validation, unchanged conversation');

...
try{
 server=spawn(path.join(root,'.venv/bin/python'),['-u','-c','from jarviss.local_board import LocalBoard; import json,sys; b=LocalBoard(); print(json.dumps(b.start("127.0.0.1"))); sys.stdin.read(); b.close()'],{cwd:root,env:{...process.env,JARVISS_DATA:temp}});
